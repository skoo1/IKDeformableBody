...
class ENVIRONMENT : public RaisimGymEnv {
 ...
 public:
  ...
  float step(const Eigen::Ref<EigenVec>& action) final {
    ...
    Eigen::VectorXd trans(3), pose(72), betas(10);
    for(int i = 0; i < 3; i++) {
    ...
    for(int i = 0; i < 10; i++) {
      betas(i) = static_cast<double>(action[75 + i]);
    }

    auto [smpl_r, _, joints] = smpl_->SMPL_Calc(trans, pose, betas);
  ...
  void build(const Eigen::Ref<EigenRowMajorMat>& vertices, const Eigen::Ref<EigenRowMajorMat>& normals) final {
    ...
    // Build SMPL Model
    {
      std::string model_path = "basicmodel_f_lbs_10_207_0_v1.1.0.json";
