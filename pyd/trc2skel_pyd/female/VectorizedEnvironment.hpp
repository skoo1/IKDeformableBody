...
class VectorizedEnvironment {
 ...
 public:
  ...
  explicit VectorizedEnvironment(std::string resourceDir, std::string cfg, bool normalizeObservation=true)
    ...
    // std::cout << "VectorizedEnvironment() 1" << std::endl;
        
    Yaml::Parse(cfg_, cfg);

    // std::cout << "VectorizedEnvironment() 2" << std::endl;

    if(&cfg_["render"])
      render_ = cfg_["render"].template As<bool>();

    // std::cout << "VectorizedEnvironment() 3" << std::endl;
  ...
  void init() {
    ...
    // std::cout << "VectorizedEnvironment.init() 1" << std::endl;
    ...
    // std::cout << "VectorizedEnvironment.init() 2" << std::endl;
    ...
    // std::cout << "VectorizedEnvironment.init() 3" << std::endl;
...
#pragma omp parallel for schedule(static)
  ...
  void build(Eigen::Ref<EigenRowMajorMat>& trc,
...
#pragma omp parallel for schedule(static)
    for (int i = 0; i < num_envs_; i++)
      perAgentBuild(i, trc, markers, indices, betas, f);
  }
...
#pragma omp parallel for schedule(static)
  ...
  inline void perAgentBuild(int agentId,
                           Eigen::Ref<EigenRowMajorMat>& trc,
                           std::vector<std::string>& markers,
                           Eigen::Ref<Eigen::VectorXi>& indices,
                           Eigen::Ref<EigenVec>& betas,
                           Eigen::MatrixXi& f) {
    environments_[agentId]->build(trc, markers, indices, betas, f);
  }
