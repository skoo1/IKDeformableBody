...
Before running the pipeline, you must generate the required `.pyd` files in the `pyd` directory.
The `.pyd` files can be generated by following the instructions provided here: [pyd_directory](https://github.com/skoo1/IKDeformableBody/tree/main/pyd)

After setting up the environment and generating the `.pyd` files, run the pipeline in the following order:
1. `scan2smpl`
2. `smpl4marker`
3. `trc2skel`
