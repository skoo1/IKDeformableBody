...
def make_env(gender: str):
    ...
    if g in ("m", "male"):
        from smpl4marker_male import RaisimGymEnv
    elif g in ("f", "female"):
        from smpl4marker_female import RaisimGymEnv
    else:
...
########## Main Begins Here ##########
def main(obj_arg: str, gender: str, betas: np.ndarray, out_root="results", scan_root="scan_files"):
    ...
    env.build(obj_vertices, obj_vertex_normals, betas)
    ...
    trans0 = np.zeros(3)
    pose0 = np.zeros(72)
    cma_param_initpose = np.concatenate([trans0, pose0])
    cma_param_sigma0 = 0.05
    cma_options = {
        'maxiter': 50000,
        'bounds': [np.concatenate([[-0.1] * len(trans0), [-np.pi] * len(pose0)]),  # 하한
                   np.concatenate([[0.1] * len(trans0), [np.pi] * len(pose0)])],  # 상한
        'verb_disp': 100,
...
if __name__ == '__main__':
    ...
    parser.add_argument("--betas",
                        type=str,
                        default=None,
                        help="SMPL shape coefficients (10 values). If omitted, zeros are used."
                    )
    ...
    if args.betas is None:
        ...
        tokens = args.betas.split()
        if len(tokens) != 10:
            raise ValueError(f"--betas must contain exactly 10 values (got {len(tokens)})")
        betas = np.array([float(x) for x in tokens], dtype=np.float32)
